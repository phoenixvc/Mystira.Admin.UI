...
export const characterMapsApi = {
  getCharacterMaps: async (
    ...
    const response = await apiClient.get<CharacterMapQueryResponse>(
  ...
  getCharacterMap: async (id: string): Promise<CharacterMap> => {
    const response = await apiClient.get<CharacterMap>(
      `/api/admin/charactermaps/${id}`
    );
    return response.data;
  },

  createCharacterMap: async (
    characterMap: Partial<CharacterMap>
  ): Promise<CharacterMap> => {
    const response = await apiClient.post<CharacterMap>(
      "/api/admin/charactermaps",
      characterMap
    );
    return response.data;
